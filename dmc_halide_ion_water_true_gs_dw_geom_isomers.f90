...
! dw_tau_max: Parameter that controls the number of DMC steps that elapse before the DW weights are reset
...
! hist_dist_min,histangle_min,hist_pairs_OO_min,hist_pairs_OX_min,hist_pairs_HX_min: Lower bound for histogram data on x axis
! hist_dist_max,hist_angle_max,hist_pairs_OO_max,hist_pairs_OX_max,hist_pairs_HX_max: Upper bound for histogram data on x axis
! nb_dist,nb_angle,nb_pairs_OO,nb_pairs_OX,nb_pairs_HX: Number of bins used to generate histgrams
! distances_angles: Logical parameter that controls if OH bond distances, HOH bond angles, and OO/OX/HX pair distances are computed
...
! inertia: Logical parameter that controls if the moments of inertia are computed when no quenching is done 
...
! potential: MB-nrg and iTTM potential energy surfaces
! halide_type: Chemical symbol for the halide ion used in the DMC simulation
...
! r_pairs_thresh: O-O and O-halide ion pair distance threshold for assignment of configurations to library reference configurations
...
! Npairs: Number of pairs of oxygen atoms and the halide ion
! Npairs_H2O: Number of pairs of oxygen atoms only
...
! d_dist,d_angle,d_pairs: Length of each histogram bin
! OH_hist(0:nb_dist,Ndw),HOH_hist(0:nb_angle,Ndw),pairs_OO_hist(0:nb_pairs_OO,Ndw),pairs_OX_hist(0:nb_pairs_OX,Ndw),pairs_HX_hist(0:nb_pairs_HX,Ndw): Arrays containing data for each histogram
...
! lib_config_cos(Npairs_H2O,Nisomers): Array to store the value of the cosines of each reference configuration
...
! pair_OO_dist: Pair distance between oxygen atoms
! pair_OX_dist: Pair distance between oxygen atom and halide ion
! pair_HX_dist: Pair distance between hydrogen atom and halide ion
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! LOCAL VARIABLES IN HISTOGRAM SUBROUTINE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! cosines(Npairs_H2O): Value of the cosine for the OH bond angles
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! LOCAL VARIABLES IN QUENCH SUBROUTINE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
module dmc_module
  ...
  Integer :: Natoms,Dim,NH2O,Npairs,Npairs_H2O,Nmax,N0,N0_part,Nt,Nt1,Nisomers
  ...
  Character(len=10) :: isotope
  Character(len=2) :: halide_type
  Character(len=7) :: file_type
  Character(len=60) :: file_num,coord_config
  ...
  Integer :: nb_dist,nb_angle,nb_pairs_OO,nb_pairs_OX,nb_pairs_HX
  Double Precision :: hist_dist_min,hist_angle_min,hist_pairs_OO_min,hist_pairs_OX_min,hist_pairs_HX_min
  Double Precision :: hist_dist_max,hist_angle_max,hist_pairs_OO_max,hist_pairs_OX_max,hist_pairs_HX_max
  Double Precision :: d_dist,d_angle,d_pairs_OO,d_pairs_OX,d_pairs_HX
  Integer, Allocatable :: OH_hist(:,:),HOH_hist(:,:),pairs_OO_hist(:,:),pairs_OX_hist(:,:),pairs_HX_hist(:,:)
  
  ! Global variables for quenching
  Integer :: grad_maxfac,count_below_thresh,count_others,count_unphysical
...
contains
  ...
  subroutine pot_energy(energy,x,Dim)
    ...
    if (potential=='mbnrg') then
       call calcpot(NH2O,energy,x) ! x in angstroms, energy in kcal/mol
    else
  ...
  subroutine gradient(grad,x,Dim)
    ...
    if (potential=='mbnrg') then
       call calcpotg(NH2O,energy,x,grad) ! x in angstroms, energy in kcal/mol, grad in kcal/mol/angstrom
    else
       stop 'unrecognized potential'
    endif
  ...
  subroutine O_halide_distances(x,pair_dist)
    Implicit None
    Integer :: i,j,l
    Double Precision, Intent(In) :: x(Dim)
    Double Precision, Intent(Out) :: pair_dist(Npairs)
    
    ! Given a configuration of NH2O water molecules and 1 halide ion, compute O-O and O-X^- pair distances
    pair_dist=0d0
    l=0
    do i=2,NH2O+1
       do j=1,i-1
  ...
  end subroutine O_halide_distances

  subroutine OH_bonds(x,cosines)
    Implicit None
    Integer :: i,j,l
    Double Precision, Intent(In) :: x(Dim)
    Double Precision, Intent(Out) :: cosines(Npairs_H2O)
    ...
    do i=1,NH2O
       ...
       enddo
    enddo
    if (Npairs_H2O>1) call hpsort(Npairs_H2O,cosines)
...
20  if (J.le.IR) then
  ...
  subroutine diffusion()
    ...
    do n=1,Nt
       ...
       ! If the potential energy of the configuration falls just below the global minimum then remove this walker from the population and write it out
       if (energy<energy_thresh) then 
          population(n)=0
          count_below_thresh=count_below_thresh+1
          write(file_num,'(i7)') count_below_thresh
          
          ! Write out the random walker that falls below the energy threshold
          coord_config='library_opt_configs/'//trim(adjustl(file_num))//'_unphysical_below_thresh.xyz'
          open(80,file=coord_config)
          write(80,*) Natoms
          write(80,*) energy,'|kcal/mol'
          do i=1,Natoms
             write(80,*) atom_type(i),x(3*i-2:3*i,n)
          enddo
          close(80)
       endif
  ...
  subroutine histogram()
    ...
    Double Precision :: OH_dist_temp,OH_dist(2),HH_dist,HOH_angle,pair_OO_dist,pair_OX_dist,pair_HX_dist
    ...
    ! Parallel Region
    !$omp parallel &
    !$omp shared (Natoms,Nt1,NH2O,Ndw,dw_x,hist_dist_min,hist_angle_min) &
    !$omp shared (hist_pairs_OO_min,hist_pairs_OX_min,hist_pairs_HX_min) &
    !$omp shared (d_dist,d_angle,d_pairs_OO,d_pairs_OX,d_pairs_HX,nb_dist,nb_angle) &
    !$omp shared (nb_pairs_OO,nb_pairs_OX,nb_pairs_HX,dw_weight) &
    !$omp private (i,j,k,l,m,n,j1,j2,OH_dist_temp,OH_dist,HH_dist,HOH_angle,pair_OO_dist,pair_OX_dist,pair_HX_dist)
    ...
    !$omp do reduction (+:OH_hist,HOH_hist,pairs_OO_hist,pairs_OX_hist,pairs_HX_hist)
    do n=1,Nt1
       if (dw_weight(1,n).ne.0) then
          ...
          ! Compute the O-O pair distances and determine the correlation function
          do i=2,NH2O
             do j=1,i-1
          ...
          ! Compute the O-X pair distances and determine the correlation function
          do i=1,NH2O
             pair_OX_dist=sum((dw_x(3*Natoms-2:3*Natoms,n)-dw_x(9*i-8:9*i-6,n))**2)
             l=(dsqrt(pair_OX_dist)-hist_pairs_OX_min)/d_pairs_OX
             if (l>nb_pairs_OX) l=nb_pairs_OX
             if (l<0) l=0
             do k=1,Ndw
                pairs_OX_hist(l,k)=pairs_OX_hist(l,k)+dw_weight(k,n)
             enddo
          enddo

          ! Compute the H-X pair distances and determine the correlation function
          do i=1,NH2O
             do j=1,2
                pair_HX_dist=sum((dw_x(3*Natoms-2:3*Natoms,n)-dw_x(9*i-8+3*j:9*i-6+3*j,n))**2)
                l=(dsqrt(pair_HX_dist)-hist_pairs_HX_min)/d_pairs_HX
                if (l>nb_pairs_HX) l=nb_pairs_HX
                if (l<0) l=0
                do k=1,Ndw
                   pairs_HX_hist(l,k)=pairs_HX_hist(l,k)+dw_weight(k,n)
                enddo
             enddo
          enddo
       endif
    ...
    do j=1,Ndw
       ...
       enddo
       do i=0,nb_pairs_OO
          write(9,*) hist_pairs_OO_min+(i+0.5)*d_pairs_OO,pairs_OO_hist(i,j)/(Npairs_H2O*dw_Nt(j)*d_pairs_OO)
       enddo
       do i=0,nb_pairs_OX
          write(10,*) hist_pairs_OX_min+(i+0.5)*d_pairs_OX,pairs_OX_hist(i,j)/(NH2O*dw_Nt(j)*d_pairs_OX)
       enddo
       do i=0,nb_pairs_HX
          write(90,*) hist_pairs_HX_min+(i+0.5)*d_pairs_HX,pairs_HX_hist(i,j)/(2*NH2O*dw_Nt(j)*d_pairs_HX)
       enddo
    enddo
    call flush(7)
    call flush(8)
  ...
  subroutine quench()
    ...
    Double Precision :: energy_opt,energy_unopt,x_quench(Dim),pair_dist(Npairs),cosines(Npairs_H2O)
    ...
    ! Parallel Region
    !$omp parallel &
    !$omp shared (grad_tol,grad_maxfac,dw_x,Dim,min_energy,Nt1,Npairs,Npairs_H2O,Nisomers,Ndw,energy_thresh) &
    ...
    do n=1,Nt1
       if (dw_weight(1,n).ne.0) then
          ...
          if (info==0.or.info==2) then 
             ...
             call O_halide_distances(x_quench,pair_dist)
             ...
             do k=1,Nisomers
                if (dabs(energy_opt-min_energy(k))<r_energy_thresh &   
                     ...
                     .and.distance(Npairs_H2O,cosines,lib_config_cos(:,k))<r_cos_thresh) then
             ...
             ! Keep track of the number of "others" unquenched and quenched configurations and write out the "others" configs to an xyz file
             count_others=count_others+1
             write(file_num,'(i7)') count_others

             ! Write out the random walker that quenches to the configuration assigned to "others"
             coord_config='library_opt_configs/'//trim(adjustl(file_num))//'_others_unopt.xyz'
             call pot_energy(energy_unopt,dw_x(:,n),Dim) ! x(:,n) in angstrom, energy in kcal/mol,'|kcal/mol',info
             open(40,file=coord_config)
             write(40,*) Natoms
             write(40,*) energy_unopt,'|kcal/mol'
             do i=1,Natoms
                write(40,*) atom_type(i),dw_x(3*i-2:3*i,n)
             enddo
             close(40)

             ! Write out the quenched configuration assigned to "others"
             coord_config='library_opt_configs/'//trim(adjustl(file_num))//'_others_opt.xyz'
             open(50,file=coord_config)
             write(50,*) Natoms
             write(50,*) energy_opt,'|kcal/mol',info
             do i=1,Natoms
                write(50,*) atom_type(i),x_quench(3*i-2:3*i)
             enddo
             close(50)
             ...
             ! Keep track of the number unphysical unquenched and quenched configurations and write out the unphysical configs to an xyz file
             count_unphysical=count_unphysical+1
             write(file_num,'(i7)') count_unphysical
             
             ! Write out the random walker that quenches to the unphysical configuration
             coord_config='library_opt_configs/'//trim(adjustl(file_num))//'_unphysical_unopt.xyz'
             call pot_energy(energy_unopt,dw_x(:,n),Dim) ! x(:,n) in angstrom, energy in kcal/mol,'|kcal/mol',info
             open(60,file=coord_config)
             write(60,*) Natoms
             write(60,*) energy_unopt,'|kcal/mol'
             do i=1,Natoms
                write(60,*) atom_type(i),dw_x(3*i-2:3*i,n)
             enddo
             close(60)
             
             ! Write out the quenched unphysical configuration
             coord_config='library_opt_configs/'//trim(adjustl(file_num))//'_unphysical_opt.xyz'
             open(70,file=coord_config)
             write(70,*) Natoms
             write(70,*) energy_opt,'|kcal/mol',info
             do i=1,Natoms
                write(70,*) atom_type(i),x_quench(3*i-2:3*i)
             enddo
             close(70)
          endif
       endif
...
Program dmc_true_gs
  ...
  read(2,*) hist_pairs_OX_min,hist_pairs_OX_max,nb_pairs_OX
  read(2,*) hist_pairs_HX_min,hist_pairs_HX_max,nb_pairs_HX
  ...
  ! Compute the number of water molecules and number of OO and O-halide ion pairs
  NH2O=(Natoms-1)/3
  Npairs=((NH2O+1)*NH2O)/2
  Npairs_H2O=(NH2O*(NH2O-1))/2
  ...
  if (histograms) then
     ...
     d_pairs_OX=(hist_pairs_OX_max-hist_pairs_OX_min)/(nb_pairs_OX+1)
     d_pairs_HX=(hist_pairs_HX_max-hist_pairs_HX_min)/(nb_pairs_HX+1)
  endif
  ...
  if (histograms) then
     ...
     allocate(pairs_OX_hist(0:nb_pairs_OX,Ndw),pairs_HX_hist(0:nb_pairs_HX,Ndw))
  endif
  if (quenching) allocate(lib_config_pd(Npairs,Nisomers),lib_config_cos(Npairs_H2O,Nisomers),count_config(Nisomers+2,Ndw))
  ...
  if (file_type=='new') then
     ...
     if (halide_type=='F') then
        write(1,*) '# Fmass=',Fmass/melectron
     else if (halide_type=='Cl') then 
        write(1,*) '# Clmass=',Clmass/melectron
     else if (halide_type=='Br') then 
        write(1,*) '# Brmass=',Brmass/melectron
     else if (halide_type=='I') then 
        write(1,*) '# Imass=',Imass/melectron
     endif
     write(1,*) '# Incr Opt=',Kaver
     write(1,*) '# Number of Reference Configs=',Nisomers
     write(1,*) '# Number of Threads=',num_threads
     if (potential=='mbnrg') write(1,*) '# Potential=MBnrg'
     ...
     if (inertia) then
        ...
        write(19,*) '# Column 2=Isomer Index'
     ...
     if (histograms) then
        ...
        open(10,file='histogram_pairs_OX_dist.dat') 
        write(10,*) '# Histogram OX Pairs=',hist_pairs_OX_min,hist_pairs_OX_max
        write(10,*) '# Histogram OX Pairs=',d_pairs_OX
        write(10,*) '# Column 1=OX Pair Distances'
        write(10,*) '# Column 2=Normalized Bin Count'

        open(90,file='histogram_pairs_HX_dist.dat') 
        write(90,*) '# Histogram HX Pairs=',hist_pairs_HX_min,hist_pairs_HX_max
        write(90,*) '# Histogram HX Pairs=',d_pairs_HX
        write(90,*) '# Column 1=HX Pair Distances'
        write(90,*) '# Column 2=Normalized Bin Count'
     ...
     if (histograms) then
        ...
        open(10,file='histogram_pairs_OX_dist.dat',status='old',position='append')
        open(90,file='histogram_pairs_HX_dist.dat',status='old',position='append')
     endif
  ...
  do i=1,Nisomers
     ...
     ! Store the pair distances and cosines of the reference configs
     if (quenching) then 
        call O_halide_distances(x(:,1+(i-1)*N0_part),lib_config_pd(:,i))
